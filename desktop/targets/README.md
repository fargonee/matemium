...
Agent workspace tools operate on approved logical paths such as `scenes.py`, `helpers.py`, `brief/tapes/main.md`, `brief/orchestration.md`, and the selected path's narration/audio artifacts; the Rust orchestrator resolves them against the workspace root with `PathBuf`.
